...
const REQUIRED_FIELDS = ["actor", "channel", "product_id", "plant_id", "customer_id"];
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const {
      actor, channel, product_id, plant_id, customer_id,
    ...
    const offer = await sql.begin(async (tx) => {
      const [offer] = await tx`
        insert into sent_offers (
          ...
          ${actor}, ${channel}, ${product_id}, ${product.name_en}, ${product.name}, ${product.full_name_en}, ${product.full_name_es},
