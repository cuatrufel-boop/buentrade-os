...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const {
      actor, name, country = null, city = null, phone = null,
    ...
    const provider = await sql.begin(async (tx) => {
      const [provider] = await tx`
        insert into providers (name, country, city, phone, contact_name, email, email_cc, whatsapp_cc, notes, mc_number, dot_number)
        values (${name}, ${country}, ${city}, ${phone}, ${contact_name}, ${email}, ${email_cc}, ${whatsapp_cc}, ${notes}, ${mc_number}, ${dot_number})
        returning *
      `;
