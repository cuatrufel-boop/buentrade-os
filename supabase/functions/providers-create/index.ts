...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (idempotency_key) {
      const [existing] = await sql`select * from providers where idempotency_key = ${idempotency_key}`;
      if (existing) return jsonResponse({ created: true, provider: existing, idempotent_replay: true });
    }
