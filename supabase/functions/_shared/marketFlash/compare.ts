...
interface Series { kind: string; metric: string; unit: (v: Record<string, any>) => string; minDays: number; maxDays: number; pre: string; post: string; dec: number }
const SERIES: Series[] = [
  { kind: "cut_price_weekly", metric: "price", unit: (v) => v.unit, minDays: 10, maxDays: 21, pre: "US$", post: "/cwt", dec: 2 },
  { kind: "mx_pork_price", metric: "price", unit: (v) => v.unit, minDays: 10, maxDays: 21, pre: "US$", post: "/kg", dec: 2 },
  { kind: "hog_price_us_mx", metric: "price", unit: (v) => v.unit, minDays: 10, maxDays: 21, pre: "US$", post: "/kg", dec: 2 },
  { kind: "futures", metric: "price", unit: () => "as printed", minDays: 10, maxDays: 21, pre: "", post: "", dec: 2 },
  { kind: "cold_storage", metric: "stocks", unit: (v) => v.unit, minDays: 25, maxDays: 40, pre: "", post: " mil lb", dec: 0 },
];
...
const num = (v: number, dec: number) => v.toLocaleString("en-US", { minimumFractionDigits: dec, maximumFractionDigits: dec });
...
export function trendBullets(current: Fact[], previous: Fact[], sourceNote: (f: Fact) => string): Bullet[] {
  ...
  for (const f of current) {
    ...
    const word = change === 0 ? "sin cambio" : `${Math.abs(change)}% ${change > 0 ? "más" : "menos"}`;
    const L = labelEs(f)!;
